...
function handleReportAll_(requestingWorkerId, workersCSV) {
  ...
  if (iWorker < 0 || iDate < 0 || iTime < 0) {
    return {
      ok: false,
      message: "ClockIn sheet missing WorkerID/Date/Time columns",
    };
  }
  ...
  const allWorkers = Object.keys(names).map((id) => ({
    id,
    name: names[id] || id,
  }));
...
function handleReportForWorker_(workerId) {
  ...
  for (let r = 1; r < values.length; r++) {
    ...
    const dateStr =
      d instanceof Date
        ? Utilities.formatDate(d, TIMEZONE, "MM/dd/yyyy")
        : String(d);
    if (dateStr !== todayISO) continue;
    const t = values[r][iTime];
    const timeStr =
      t instanceof Date
        ? Utilities.formatDate(t, TIMEZONE, "hh:mm:ss a")
        : String(t || "");
...
function getPayrollWeekPeriods_(workerId) {
  try {
    ...
    if (!sheet) {
      ...
      return {
        success: false,
        message: "❌ Payroll LineItems sheet not found.",
      };
    }
    ...
    if (iWorker < 0 || iWeek < 0) {
      ...
      return {
        success: false,
        message: "❌ Missing WorkerID or Week Period columns",
      };
    }
...
function generatePayrollPdf_(workerId, workerName, weekEndISO) {
  ...
  const body = `Attached is the payroll report for ${
