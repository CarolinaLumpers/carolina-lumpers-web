  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (String(row[emailIdx]).toLowerCase() === email) {
      ...
      if (row[passIdx] === hash) {
        // Successful login
        Logger.log(
          `✅ Login successful for ${email} (${workerId}) - ${displayName}`,
        );

        // Get W-9 status information
        const w9Status = w9StatusIdx >= 0 ? row[w9StatusIdx] || "none" : "none";
        const w9SubmittedDate =
          w9SubmittedDateIdx >= 0 ? row[w9SubmittedDateIdx] : "";
        const w9ApprovedDate =
          w9ApprovedDateIdx >= 0 ? row[w9ApprovedDateIdx] : "";
...
function signUpUser(e) {
  ...
  if (!isValidSignupPassword_(password)) {
    TT_LOGGER.logSystem(`Signup failed: Weak password - ${email}`, {
      status: "WARNING",
      details: {
        email: email,
        device: device,
        reason: "Password policy failed",
      },
    });
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (String(row[emailIdx]).toLowerCase() === email) {
      ...
      const workerId =
        workerIdIdx >= 0 ? String(row[workerIdIdx] || "").trim() : "PENDING";
      const displayName =
        displayNameIdx >= 0
  ...
  // Email not found
  Logger.log(
    `❌ Password setup failed for ${email} - Email not found in Workers sheet`,
  );
  ...
  TT_LOGGER.logSystem(`Signup failed: Email not found - ${email}`, {
    ...
    details: {
      email: email,
      device: device,
      reason: "Email not in Workers sheet",
    },
  });
...
function isValidSignupPassword_(password) {
  return (
...
function getWorkerIdByEmail(email) {
  ...
  if (iEmail < 0 || iWorker < 0) {
    return {
      success: false,
      message: "Workers sheet missing Email/WorkerID columns.",
    };
  }

  const search = String(email || "")
    .trim()
    .toLowerCase();
  for (let r = 1; r < data.length; r++) {
    const rowEmail = String(data[r][iEmail] || "")
      .trim()
      .toLowerCase();
    if (rowEmail === search) {
      const workerId = String(data[r][iWorker] || "").trim();
      const displayName =
        iName >= 0 && data[r][iName] ? String(data[r][iName]).trim() : workerId;
      return {
        success: true,
        workerId,
        displayName,
  ...
  for (let r = 1; r < data.length; r++) {
    if (String(data[r][iWorker]) === String(workerId)) {
      return {
        ...
        workerId,
        displayName:
          iDisplay >= 0 ? String(data[r][iDisplay] || "").trim() : workerId,
...
function getWorkerNames_() {
  ...
  for (let r = 1; r < data.length; r++) {
    ...
    if (iAvailability >= 0) {
      const availability = String(data[r][iAvailability] || "")
        .trim()
        .toLowerCase();
    ...
    const name =
      display || [first, last].filter(Boolean).join(" ").trim() || id;
    map[id] = name;
  }
...
function normalizeRoleName_(rawRole) {
  ...
  if (role === "worker" || role === "user" || role === "none" || !role)
    return "Worker";
